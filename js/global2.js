$(document).ready(function()
	{
		...
		$('#airplane a').click(function()
			{
				$('#airplane a').removeClass('selected');
				// Asign value of the link target
				var thisTarget = $(this).attr('href');
				var retorno = thisTarget.split("_");
				//alert(retorno[1]);
				//alert (thisTarget);
				...
				//alert (thisSeat);
				var preseat = thisSeat.split("_");
				var posseat = preseat[1].split(" ");
				var escolha = posseat[0]+posseat[1];
				escolhafinal = escolha;
				//alert (escolha);
				//alert('Mudança de assento sujeita a disponibilidade no momento do Check-In\\nATENÇÃO!!!\\nCaso não seja confirmado o pagameno este assento voltará a estar disponível para esse voo.');
		...
		$('form').submit(function()
			{
				...
				//var nome = $("#nome").val();
				$("#seat").val(escolhafinal);
				var testeSeat = $("#seat").val();
				if(testeSeat == ""){
					alert("Por Favor escolha um assento!!!");
					return false;
				}else{
					//alert(testeSeat);
					//alert('Assento Escolhido: ' + escolhafinal +'\\nQuem esoclheu: ' + nome);
					//aqui não deixa fazer a action return false;
					return true;
				}
